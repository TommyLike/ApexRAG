...
.PHONY: start
start:
	@go run -ldflags "-X main.VERSION=$(RELEASE_TAG)" ./main/main.go web -c ./configs/config.toml
...
.PHONY: docker-push
docker-push:
	time docker build --file ./Dockerfile --no-cache --tag tommylike/apaxrag .
	docker login
	docker push tommylike/apaxrag
